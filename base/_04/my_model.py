...
class CLIPComplete(nn.Module):
    def __init__(self, model_visual):
        ...
        self.proj1 = nn.Parameter(width ** -0.5 * torch.randn(width, output_dim))
        self.proj2 = nn.Parameter(width ** -0.5 * torch.randn(width, output_dim))
    ...
    def forward(self, img1, img2):
        img1_features = self.encode_image(img1, self.visual)
        img2_features = self.encode_image(img2, self.visual)
        
        img1_features = img1_features @ self.proj1.type(self.dtype)
        img2_features = img2_features @ self.proj2.type(self.dtype)
